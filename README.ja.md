...
本プロジェクトのコード・ドキュメント・設計はすべて AI アシスタント [Claude Sonnet 4.6](https://www.anthropic.com/claude)（Anthropic）によって作成されました。会話形式の開発セッションを通じて生成されたものです。

## 注意事項

> **警告**: 本ツールは回路図ファイルに直接変更を加えます。実行する前に必ず回路図ファイルのバックアップを保存してください。
